...
public class SistemaMonitoramentoRepository {
    ...
    public List<SistemaConfigDTO> listarConfiguracoes() throws SQLException {
        ...
        try (Connection conn = ConnectionFactory.getConnection();
            ...
            while (rs.next()) {
                lista.add(new SistemaConfigDTO(
                    rs.getString("chave"),
                    rs.getString("valor_config"),
    ...
    public List<JobExecucaoDTO> listarJobsAgendados() throws SQLException {
        ...
        try (Connection conn = ConnectionFactory.getConnection();
             ...
             ResultSet rs = stmt.executeQuery()) {
            while (rs.next()) {
                lista.add(new JobExecucaoDTO(
                    rs.getLong("id"),
                    rs.getString("tipo_job"),
                    rs.getString("status"),
    ...
    public List<OutboxEventDTO> listarEventosOutbox(Integer status) throws SQLException {
        ...
        try (Connection conn = ConnectionFactory.getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    lista.add(new OutboxEventDTO(
                        rs.getLong("id"),
                        rs.getString("tenant_id"),
                        rs.getString("evento_tipo"),
                        rs.getString("payload"),
                        rs.getInt("processado"),
    ...
    public List<SchemaVersionDTO> listarVersoesSchema() throws SQLException {
        ...
        try (Connection conn = ConnectionFactory.getConnection();
             ...
             ResultSet rs = stmt.executeQuery()) {
            while (rs.next()) {
                lista.add(new SchemaVersionDTO(
                    rs.getLong("id"),
                    rs.getString("versao"),
                    rs.getString("descricao"),
                    rs.getString("executado_por"),
