...
import br.com.yat.ecosystemcore.modules.usuario.entity.EmpresaUsuario;
...
public class EmpresaUsuarioRepository extends GenericDao<EmpresaUsuario, String> {
	...
	public void vincular(Connection conn, String tenantId, Long empresaId, Long usuarioId, Long perfilId)
			throws SQLException {
	...
	/**
	 * 🔥 NOVO MÉTODO ACOPLADO À SUA IDEIA: Carrega a visão detalhe realizando a
	 * junção de tabelas (JOIN) exigida pela tabela reativa do JavaFX, isolando por
	 * Tenant e Usuário Ativo.
	 */
	public List<EmpresaUsuarioDetalheDTO> findVinculosPorUsuario(Connection conn, Long usuarioId, String tenantId)
			throws SQLException {
		String sql = """
		...
		try (PreparedStatement stmt = conn.prepareStatement(sql)) {
			...
			try (ResultSet rs = stmt.executeQuery()) {
				while (rs.next()) {
					lista.add(new EmpresaUsuarioDetalheDTO(rs.getLong("empresa_id"), rs.getString("empresa_nome"),
							rs.getLong("perfil_id"), rs.getString("perfil_nome")));
				}
			}
		}
		return lista;
	...
	public List<CheckBoxListCellData> findAllPermissoes(Connection conn) throws SQLException {
		...
		try (PreparedStatement stmt = conn.prepareStatement(sql); ResultSet rs = stmt.executeQuery()) {
			while (rs.next()) {
				lista.add(new CheckBoxListCellData(rs.getLong("id"), rs.getString("chave_composta"), // Use o nome exato
																										// do banco
	...
	public void removerTodosVinculosDoUsuario(Connection conn, String tenantId, Long usuarioId) throws SQLException {

		String sql = """
				DELETE FROM empresa_usuario
				WHERE usuario_id = ?
				AND tenant_id = ?
				""";

		executeUpdate(conn, sql, usuarioId, tenantId);
	}
}