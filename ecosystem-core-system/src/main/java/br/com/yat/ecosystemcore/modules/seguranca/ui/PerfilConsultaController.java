...
public class PerfilConsultaController implements Initializable, ScreenLifecycle, ContextAware {
	...
	@FXML
	private TableColumn<Perfil, Long> colId;
	@FXML
	private TableColumn<Perfil, String> colNome;
	@FXML
	private TableColumn<Perfil, String> colChave;
	@FXML
	private TableColumn<Perfil, String> colDescricao;
	@FXML
	...
	private void configurarListeners() {
		...
		txtPesquisa.textProperty().addListener((observable, oldValue, newValue) -> {
			filteredData.setPredicate(perfil -> {
				...
				// Transforma as propriedades textuais em um fluxo de dados e verifica se alguma
				// bate com a busca
				return java.util.stream.Stream
						.of(perfil.getNome(), perfil.getChaveIdentificadora(), perfil.getDescricao())
						.filter(java.util.Objects::nonNull) // Ignora campos nulos no banco com segurança
						.map(String::toLowerCase).anyMatch(campo -> campo.contains(query)); // Retorna true se houver
																							// match em qualquer campo
			});
		});
	}
	...
	public void carregarDados() {
		AppExecutors.execute(() -> {
			try {
				var lista = perfilService.listarPerfisPorTenant();
				...
				Platform.runLater(() -> mostrarAlerta("Erro", "Falha ao carregar perfis: " + e.getMessage(),
