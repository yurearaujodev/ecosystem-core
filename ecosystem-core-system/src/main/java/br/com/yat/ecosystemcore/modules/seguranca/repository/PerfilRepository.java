package br.com.yat.ecosystemcore.modules.seguranca.repository;
...
public class PerfilRepository extends GenericDao<Perfil, Long> {
	...
	public Long criarPerfilAdminSeNecessario(Connection conn, String tenantId, Long usuarioCriadorId)
			throws SQLException {
		String sql = """
		...
		Long idGerado = executeInsertReturningId(conn, sql, UUID.randomUUID().toString(), tenantId, "Administrador",
				"ADMIN", "Acesso total ao sistema", usuarioCriadorId // O bindParameters já trata o nulo de forma
																		// nativa!
		);
	...
	public void salvar(Connection conn, Perfil perfil) throws SQLException {
		String sql = """
				    INSERT INTO perfil (uuid_publico, tenant_id, nome, chave_identificadora, descricao,version, created_by)
				    VALUES (?, ?, ?, ?,?, ?, ?)
				""";
		Long idGerado = executeInsertReturningId(conn, sql, perfil.getUuidPublico(), perfil.getTenantId(),
				perfil.getNome(), perfil.getChaveIdentificadora(), perfil.getDescricao(), perfil.getVersion(),
				perfil.getCreatedBy());
		if (idGerado != null) {
	...
	public void atualizar(Connection conn, Perfil perfil) throws SQLException {
		String sql = """
				...
				""";
		executeUpdate(conn, sql, perfil.getNome(), perfil.getChaveIdentificadora(), perfil.getDescricao(),
				perfil.getUpdatedBy(), perfil.getId(), perfil.getTenantId());
	}
	...
	/**
	 * Sobrescreve o softDelete original para injetar o ID do usuário que realizou a
	 * deleção
	 */
	public boolean softDeleteComAuditoria(Connection conn, Long id, String tenantId, Long usuarioDeletouId)
			throws SQLException {
		String sql = """
				    UPDATE perfil
