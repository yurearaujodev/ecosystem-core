package br.com.yat.ecosystemcore.modules.cadastro.repository;
...
public class EmpresaRepository extends GenericDao<Empresa, Long> {
	...
	// Insere usando seu método utilitário que captura a Primary Key
	// auto-incrementada
	public Long insert(Connection conn, Empresa empresa) throws SQLException {
		...
		return executeInsertReturningId(conn, sql, uuid, empresa.getTenantId(), empresa.getRazaoSocial(),
				empresa.getNomeFantasia(), empresa.getCnpj(), empresa.getInscricaoEstadual(), empresa.getTelefone(),
				empresa.getLogradouro(), empresa.getNumero(), empresa.getBairro(), empresa.getCidade(),
	...
	public boolean update(Connection conn, Empresa empresa) throws SQLException {
		...
		int linhasAfetadas = executeUpdate(conn, sql, empresa.getRazaoSocial(), empresa.getNomeFantasia(),
				empresa.getCnpj(), empresa.getInscricaoEstadual(), empresa.getTelefone(), empresa.getLogradouro(),
				empresa.getNumero(), empresa.getBairro(), empresa.getCidade(), empresa.getEstado(), empresa.getCep(),
				empresa.getAtivo(), empresa.getUpdatedBy(), empresa.getId(), empresa.getTenantId(),
				empresa.getVersion());

		return linhasAfetadas > 0;
	}

	// Soft Delete refinado atualizando quem realizou a exclusão técnica
	// (deleted_by)
	...
	public Optional<Empresa> findByIdAndTenant(Connection conn, Long id, String tenantId) throws SQLException {
	...
	public List<Empresa> listarTodasAtivas(Connection conn) throws SQLException {
	    // Busca todas as empresas ativas para popular o combo de login
	    String sql = "SELECT * FROM empresa WHERE ativo = 1 AND deleted_at IS NULL ORDER BY nome_fantasia ASC";
